...
export function Hero() {
  ...
  const drawFrame = useCallback((index: number) => {
    ...
    if (isMobile) {
      // frames-mobile são 9:16 — contain preenche a tela sem cortar
      ...
      // frames desktop são 16:9 — cover preenche sem barra preta
      if (canvasRatio > imgRatio) {
        drawW = cw;
        drawH = cw / imgRatio;
      } else {
  ...
  return (
    <section ref={sectionRef} className="scroll-animation relative">
      ...
      >
        ...
        >
          <EyebrowBadge>ELECTRO GAMES</EyebrowBadge>
          <div className="flex flex-col gap-1">
            <p className="font-sans text-base font-medium text-zinc-400 md:text-lg">
              Loja Gamer &amp; Serviços
            </p>
            <h1 className="font-sans text-2xl font-bold leading-tight tracking-tighter text-foreground md:text-3xl">
              Consoles · Reparo · <span className="text-accent">Upgrades.</span>
            </h1>
          </div>
        </div>
        ...
        >
          <span className="inline-flex items-center gap-2.5 font-mono text-[10px] uppercase tracking-[0.3em] text-accent">
            <span aria-hidden className="inline-block h-1.5 w-1.5 rounded-full bg-accent shadow-[0_0_10px_rgba(57,255,20,0.85)]" />
            Missão &mdash; Level Up
          </span>
          <h2 className="font-sans font-semibold leading-[0.88] tracking-tighter text-foreground text-[clamp(4rem,9.5vw,9rem)]">
            Jogue
            <br />
            no <span className="text-accent">próximo nível.</span>
          </h2>
          <p className="max-w-[36ch] font-mono text-[11px] uppercase tracking-[0.22em] text-zinc-400">
            Os melhores títulos &amp; equipamentos, direto pra você.
          </p>
        </div>

        <div className="pointer-events-none absolute left-6 top-20 z-10 hidden items-center gap-2 md:flex md:left-10 md:top-24">
          <div className="h-px w-8 bg-accent/60" />
          <span className="font-mono text-[10px] uppercase tracking-[0.32em] text-zinc-400">
            Electro Games &mdash; Online
          </span>
        </div>

        <div className="pointer-events-none absolute right-6 top-20 z-10 hidden items-center gap-3 md:flex md:right-10 md:top-24">
          <span className="font-mono text-[10px] uppercase tracking-[0.32em] text-zinc-400">
            Estoque
          </span>
          <span
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10">
          ...
          <div className="mx-6 flex items-center justify-between pb-4 font-mono text-[10px] uppercase tracking-[0.28em] text-zinc-500 md:mx-10">
            <span>SEQ 001 / 204</span>
            <span className="hidden md:inline">ELECTRO GAMES</span>
            <span>Deslize &darr;</span>
          </div>
        </div>
        ...
        {!loaded && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-5 bg-background px-6">
            <EyebrowBadge>ELECTRO GAMES // CARREGANDO ARENA</EyebrowBadge>
            ...
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-zinc-500">
              Inicializando Arena &nbsp;&middot;&nbsp; {Math.round(loadProgress * 100)}%
            </p>
          </div>
        )}
      </div>
